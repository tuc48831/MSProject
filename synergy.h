...
/*                 This file contains deprecated legacy structures. JYS2020*/
/*.........................................................................*/
...
int connectNode(sng_int16, sng_int32);
...
#define SUCCESS             1
#define FAILURE             0
#define MATCH_DELETE        1
#define MATCH_CONTINUE      2
#define MATCH_END           3
...
#define MAX_CMD 1024
/*
  CID specific data (deprecated. June 2020. Justin Y. Shi)
*/
...
struct shell_in_t {
	char cmd[MAX_CMD];	// CMD string
	sng_int32 host;        // Return host
	sng_int16 port;		// Return port
} shell_in;
...
#define TSH_OP_MAX                435	// Including OpShell,UVRInit,UVRPing, UVRAddNode,UVRDelNode,UVRFuse
#define TSH_OP_CNT                35  // Extended from 15

#define TSH_OP_PUT                401
#define TSH_OP_GET                402
#define TSH_OP_READ               403
#define TSH_OP_PURGE              404	// Purge all tuples and requests
#define TSH_OP_SHELL              405 // Shell services
#define TSH_OP_EXIT               406 // Quit tsh
#define TSH_OP_UVRInit            407 // UVR initialization phase 1
#define TSH_OP_UVRInit2           408 // UVR initialization phase 2
#define TSH_OP_UVRPing            409 // Round-trip UVR ping
#define TSH_OP_UVRPing2           410 // Ping phase 2
#define TSH_OP_UVRPut2            411 // Phase 2 put
#define TSH_OP_UVRGet2            412 // Agressive get
#define TSH_OP_UVRPurge           413 // UVR-facing OP_PURGE
#define TSH_OP_UVRFix             414 // Repair UVR when node failure detected
#define TSH_OP_UVRPut3            415 // Decentralized PUT implementation

#define FSH_OP_MKDIR              416 // Decentralized UFS umkdir
#define FSH_OP_RMDIR              417 // Decentralized UFS urmdir
#define FSH_OP_CD                 418 // Decentralized UFS ucd
#define FSH_OP_CP                 419 // Decentralized UFS ucp
#define FSH_OP_FPUT               420 // Decentralized UFS uput
#define FSH_OP_FGET               421 // Decentralized UFS uget
#define FSH_OP_RM                 422 // Decentralized UFS urm
#define FSH_OP_LS                 423 // Decentralized UFS uls (listing)
#define FSH_OP_SYNC               424 // Sync ufs meta data to storage
#define FSH_OP_RESYNC             425 // Non-stop resync disconnected nodes back to UFS: sync + pause
#define FSH_OP_QUERY              426 // UVR propogation of ufs_mkdir
#define FSH_OP_LOADINDEX          427 // UVR load index fix for UVRInit
#define FSH_OP_RM2                428 // UVR propogation of ufs_rm
#define FSH_OP_FPUT2              429 // UVR propogation of ufs_fput
#define FSH_OP_FPUT3              430 // UVR propogation of ufs_fget
#define FSH_OP_FGET2              431 // UVR propogation of ufs_cp
#define FSH_OP_RESUME             432 // Resume FUVR operations
#define FSH_OP_RESYNC2            433 // UFS recovery
#define FSH_OP_DEL                434 // Remove a partition
#define FSH_OP_ADD                435 // Add a partition
/* The following OPCODES are deprecated and reserved for MPL future re-impl. */
...
// These codes are still in the programs
...
typedef struct {
  char GUID[TUPLENAME_LEN];
  int  type; // 0: directory, 1: file, 2: bigFile
  char name[TUPLENAME_LEN];
  char outName[TUPLENAME_LEN];
  sng_int32 size;  // File size
  sng_int32 blockSize; // size of file blocks
  sng_int16 permissions; // Doubled for partitionNo
  char upwd[TUPLENAME_LEN]; // ufs pwd
  char pwd[TUPLENAME_LEN];  // User space pwd for output delivery
  sng_int32 consoleHost;  // Return host
  sng_int32 nextHost;
  sng_int16 consolePort;  // Return port
  int partitions;  // Total number of partitions for -S
  int TTL; // Time to live. -1 to launch UVR traversal
  int nRep; // # of replicas
  int Reserve; // Reserved GB per node
  time_t timeStamp;
  char salt[TUPLENAME_LEN]; // For encrypted file system
  char primaryMark;
} ufs_mkdir_it, ufs_put_it;
...
typedef struct {
  ...
  int metaCnt;
  int TTL;
} ufs_mkdir_it2, ufs_sync_it, ufs_loadIndex;

typedef struct {
  char name[TUPLENAME_LEN];  // Default -rf
} ufs_rmdir_it, ufs_get_it;

typedef struct {
  int UID;  // UID for authorization
  char appID[TUPLENAME_LEN] ; // Application name
  sng_int32 host ;  // Client node IP
  sng_int16 port ;  // Client return port
  int pid;
} uvr_init_it, uvr_ping_it;
...
typedef struct {
  ...
  int totalCores;
  sng_int32 nextHost;
  char message[TUPLENAME_LEN*10] ; // 128 chars
  int status;
} uvr_init_ot, uvr_ping_ack, ufs_ls_ack; // uvr_init_ack. Single or multiple node uvr

typedef struct {
  char hostIP[TUPLENAME_LEN];
  sng_int32 freeStorage;
  sng_int32 totalStorage;
  int cores;
} uvr_resync_out;

typedef struct { // UVR Phase 2 header packet. The payload is # of nodes
  int TTL;  // Time to Live = total live nodes then decrement to zero
  int totalNodes; // For initial loading hostlist
  int totalCores;  // For cross checking
  char appID[TUPLENAME_LEN] ; // ApplicationID = IP+UID+Time
} uvr2_init_it;  // This is phase2 header packet has a cascading effect as it travels through UVR

typedef struct {
  sng_int32 bavail; // in GB
  sng_int32 btotal; // in GB
  int pauseStatus;  // Query onPause setting
  int totalNodes;
  int R; // Calculated R from # of partitions on node
  int partitionNo; // GUID-i partition
  int primaryMark;
  int blockID; 
  char GUID[TUPLENAME_LEN];
} loadIndex_it;
...
typedef struct { // UVR Phase 2 header packet. The payload is # of nodes
  int TTL;  // Time to Live = total live nodes then decrement to zero
  sng_int32 retHost; // Initiating UVR Fix node
  sng_int16 retPort; // Initiating UVR Fix port
  int totalNodes; // For initial loading hostlist
  int totalCores;  // For cross checking
  char appID[TUPLENAME_LEN] ; // ApplicationID = IP+UID+Time
} uvr_fix_it;  // This is phase2 header packet has a cascading effect as it travels through UVR

typedef struct { // UVR Phase 2 packet. The payload is # of nodes
  int UID;  // UID for authorization
  char appID[TUPLENAME_LEN] ; // Application name
  sng_int32 initialHost;  // Initial node IP
  int pid;
  char hostIP[TUPLENAME_LEN];  // NodeIP
  int cores; // For initial loading hostlist
  sng_int32 bavail; // in GB
  sng_int32 btotal; // in GB
} uvr2_init2_it;  // This is uvrInit phase 2 payload packet

typedef struct {
  int TTL;
  int touchCount;  // +1 at each node
  char signatures[TUPLENAME_LEN*10];  // Not used
} uvr2_ping_it;

typedef struct { // Ack from downstream
  int status;  // Should be TTL-1 or negative errors
  int totalNodes; // The best infor so far
  int totalCores; // The best infor so far
  char message[TUPLENAME_LEN*10] ; // 128 chars
} uvr2_init_ack, uvr2_ping_ack, tsinit_ack;
...
typedef struct {
  ...
  sng_int16 priority ;
  sng_int32 host ;  // Initial node's IP
  sng_int16 port ;  // Initial node's port
  int proc_id;  // For future Tuple space multiple app state isolation
  int TTL;
} tsh_put2_it ;  // Put2 UVR traversal protocol

typedef struct {
  sng_int32 host ;  // Matching node's IP
  sng_int16 port ;  // Matching node's port
  int request;    //
  int status;       // Success = match
} tsh_put3_it;  // OpPut UVR next Match result

typedef struct {  // this_op carries read or get
  char appid[NAME_LEN] ;
...
typedef struct {
  ...
  sng_int32 host ;
  sng_int16 port ;
  int request;    // Forward this_op
  sng_int32 len  ;
  int proc_id;
  int TTL;
} tsh_get2_it;  // Get2 UVR traversal protocol

typedef struct {
  sng_int32 host ;  // Matching node's IP
  sng_int16 port ;  // Matching node's port
  int status;       // Success = match
} tsh_get3_it;  // OpPut UVR next Match result
