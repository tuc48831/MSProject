...
//     P = cnf_getP();
     ...
     sprintf(tpname,"start");
     status = tsput(tpname,&o_start_tuple,tplength);

     /* setup global minimum tuple */
     t_global_minimum = LONG_MAX;
     tplength = sizeof(long);
     sprintf(tpname,"g_min");
     status = tsput(tpname,&t_global_minimum,tplength);
     ...
     if (!tour_found)
        ...
        while (!empty())
          ...
          i++;
          sprintf(tpname,"node%d",i);
          tplength = sizeof(struct node_tuple);
          status = tsput(tpname,&o_node_tuple,tplength);
          }
        ...
        while (tuples_processed < tuples_sent)
          {
          sprintf(tpname,"f*");
          status = tsget(tpname,&finished_tuple,0);
          tuples_processed++;    
          } 

        /* all workers are finished, so output minimum cost, tour, etc */
        sprintf(tpname,"g_min");
        status = tsget(tpname,&t_global_minimum,0);

        sprintf(tpname,"best_tour");
        status = tsget(tpname,best_tour,0);

        sprintf(tpname,"eff_calcs");
        status = tsget(tpname,&temp_calcs,0);
     ...
     fprintf(outfile,"Best tour cost: %ld. \npath:",t_global_minimum);
     for (j=1;j<=n;j++)
