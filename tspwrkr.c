...
int main(int argc, char **argv)
 ...
 //    ts2 = cnf_open("nodes",0);
     ...
     strcpy(tpname,"start");
     status = tsread(tpname,&i_start_tuple,0);
     n = i_start_tuple.n;
     ...
     strcpy(tpname,"g_min");
     status = tsread(tpname,&local_minimum,0);
    
     /* initialize queue, retrieve partial tour from tuple space
	(non blocking retrieval) */
     init_q();
     strcpy(tpname,"node");
     status = tsget(tpname,&i_node_tuple,0);
     if (status == 0) {
	...
	//cnf_term();
     ...
     while (TRUE)
       ...
       status = tsput(ts2,hold_tpname,&processed_tuple,sizeof(char));    
       ...
       if (tour_found)
          ...
          if (local_minimum < t_global_minimum)
             {
             strcpy(tpname,"best_tour");
             status = tsget(tpname,temp_tour,0);  

             strcpy(tpname,"eff_calcs");
             status = tsget(tpname,&temp_calc,0);  
             ...
             {
             strcpy(tpname,"g_min");
             status = tsput(tpname,&t_global_minimum,sizeof(long));  
             }
          }              
       ...
       /* attempt to retrieve another partial tour from the tuple space,
          ...
          terminate */
       strcpy(tpname,"node");
       status = tsget(tpname,&i_node_tuple,-1);
       strcpy(hold_tpname,tpname);
